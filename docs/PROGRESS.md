...
続くセッションでは、まず`BulletinFn`/`ToggleMessageReactionFn`のバンドルサイズ肥大化バグ
（Phase 12の副次的発見、37.参照）を修正した。その後ユーザーからモバイル版チャット画面で
メッセージ入力欄がキーボードに隠れるという報告を受け、原因調査の過程で**このセッションの
iOSシミュレータ自動操作ツール（前セッションとは別ツール）でもTextInputへのタップ→
フォーカス→文字入力が一貫して機能しない**という制約を新たに確認した（画面遷移用のタップは
機能する点は前セッションから改善。38.参照）。修正自体（`KeyboardAvoidingView`追加）はコード
調査のみで完了させ、ユーザー自身にシミュレータを直接操作してもらう形で動作確認した。
同種の症状が掲示板・カレンダー編集画面にもあるとの追加報告を受けて同じパターンで横展開し、
さらに「Web版チャット・掲示板で改行が消える」という報告（実際は改行自体は正しく保存されて
おり`white-space: pre-wrap`指定漏れによる表示側の問題だった、39.参照）も解消した。
**続けてPhase 13（実際のモバイルプッシュ配信）に着手・完了した**：証明書管理が不要な
Expo Push Notification Service方式を採用（設計書のPinpoint/SNS想定からの意図的な逸脱）、
EASプロジェクトを新規作成し、`deliverPush.ts`（新規Lambda、Phase 5・6で作られていたが
サブスクライバーが1つも無く実際には一度も配信されていなかったSNSトピックを購読）を実装、
`OnConnect-dev`へデプロイしてExpo Push APIへの実リクエスト発生・トークンプルーニングまで
実機（実AWS環境）で確認した（40.参照）。**これでPhase 1〜13が全て完了し、
`OnConnect-dev`へのデプロイも完了している**（直近コミット`11f7b3d`）。
...
- **Phase 13（実際のモバイルプッシュ配信、Expo Push Notification Service）**：完了・
  `OnConnect-dev`へデプロイ済み。`deliverPush.ts`が実際にExpo Push APIへリクエストを送ること・
  トークンプルーニングが動作することを実機（実AWS環境）で確認済み。**実機（本物のAPNs/FCM経由）
  でのプッシュ受信そのもの、通知タップでの画面遷移は未検証**。詳細は上記0章・下記3章40.参照
  （実装計画は`/Users/ikkounobuyuki/.claude/plans/luminous-enchanting-jellyfish.md`参照）
